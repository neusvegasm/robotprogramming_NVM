...
class TurtleSquareNode : public rclcpp::Node {
 ...
 private:
  ...
  void timer_callback() {
    ...
    RCLCPP_INFO(this->get_logger(), "Turtle Velocity: x = %.2f, z = %.2f", v, w);
    ...
    if (w == 0.0 && current_time > side_length / trans_vel ){
      v = 0.0;
      w = rot_vel;
      current_time = 0;
    }

    if (v == 0.0 && current_time > angle_to_turn / rot_vel){
      v=trans_vel;
      w=0.0;
      current_time = 0;
    }
  ...
  void pose_callback(const turtlesim::msg::Pose::SharedPtr msg) {
    ...
    RCLCPP_INFO(this->get_logger(), "Turtle Pose: x = %.2f, y = %.2f, theta = %.2f", x, y, theta);
